...
2. Secrets leak through thinking text and tool args into the store (ticket 05).
...
- Retention and pruning once a store gets large (months of sessions, one repo).
...
- Promoting bash-driven mutations (`sed -i`, `rm`, `mv`, codegen) to real `touched` edges.
  ...
  not sharp enough to ticket until 13 shows how badly stale rationale actually hurts.
